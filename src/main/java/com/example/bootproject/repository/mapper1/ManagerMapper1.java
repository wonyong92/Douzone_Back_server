...
public interface ManagerMapper1 {
    ...
    */
    ...
    @Select("SELECT ar.employee_id as employeeId, ar.attendance_appeal_request_id as attendanceAppealRequestId, ar.status as status, ar.reason, ar.attendance_info_id as attendanceInfoId, ar.appealed_start_time as appealedStartTime, ar.appealed_end_time as appealedEndTime, ar.attendance_appeal_request_time as attendanceAppealRequestTime, ar.reason_for_rejection as reasonForRejection, e.name FROM attendance_appeal_request ar INNER JOIN employee e ON ar.employee_id = e.employee_id JOIN attendance_info ai ON ar.attendance_info_id = ai.attendance_info_id WHERE (e.employee_id LIKE CONCAT('%', #{searchParameter}, '%'))  ORDER BY ar.attendance_appeal_request_time DESC LIMIT #{size} OFFSET #{startRow}")
    ...
    @Select("SELECT ar.employee_id as employeeId, ar.attendance_appeal_request_id as attendanceAppealRequestId, ar.status as status, ar.reason, ar.attendance_info_id as attendanceInfoId, ar.appealed_start_time as appealedStartTime, ar.appealed_end_time as appealedEndTime, ar.attendance_appeal_request_time as attendanceAppealRequestTime, ar.reason_for_rejection as reasonForRejection, e.name FROM attendance_appeal_request ar INNER JOIN employee e ON ar.employee_id = e.employee_id JOIN attendance_info ai ON ar.attendance_info_id = ai.attendance_info_id WHERE (e.name LIKE CONCAT('%', #{searchParameter}, '%'))  ORDER BY ar.attendance_appeal_request_time DESC LIMIT #{size} OFFSET #{startRow}")
