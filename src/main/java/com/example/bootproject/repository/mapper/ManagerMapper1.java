...
public interface ManagerMapper1 {
    ...
    //타사원의조정요청내역
    @Select({
            "SELECT attendance_appeal_request_id, status, reason, attendance_info_id, " +
                    "appealed_start_time, appealed_end_time, employee_id, " +
                    "attendance_appeal_request_time, reason_for_rejection " +
                    "FROM attendance_appeal_request " +
                    "WHERE employee_id = #{employeeId} " +
                    "ORDER BY " + "${sort} ${desc} " + // 변수로 삽입
    ...
    @Select("SELECT " +
            ...
            "LIMIT #{size} OFFSET #{startRow}")
    List<AttendanceInfoResponseDto> selectmanagerAttendanceByDate(@Param("attendanceDate") LocalDate attendanceDate,
                                                           @Param("employeeId") String employeeId,
                                                           @Param("sort") String sort,
                                                           @Param("desc") String desc,
                                                           @Param("size") int size,
    ...
    List<AttendanceInfoResponseDto> selectmanagerAttendanceByMonthAndEmployee(int year, int month, String employeeId, String sort, String desc,
