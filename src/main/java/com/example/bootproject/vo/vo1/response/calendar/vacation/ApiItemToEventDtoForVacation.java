...
public class ApiItemToEventDtoForVacation {
    @JsonAlias("attendanceStatusCategory")
    String title;
    @JsonAlias("startTime")
    String date;
    String backgroundColor = "red";
    ExtendPropsForHoliday extendedProps = new ExtendPropsForHoliday("vacation");
     ...
     *
     * */
    public ApiItemToEventDtoForVacation(LinkedHashMap dto) {
        title = dto.get("dateName").toString();
        date = new StringBuilder(dto.get("startTime").toString().split("T")[0]).toString();
    }
}
