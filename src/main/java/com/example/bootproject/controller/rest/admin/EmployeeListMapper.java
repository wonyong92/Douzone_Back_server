...
public interface EmployeeListMapper {
    ...
    @Select("(SELECT employee_id FROM employee WHERE attendance_manager = #{isManager} and ((#{searchText} REGEXP '^[0-9]+$' AND employee_id like concat('%',#{searchText},'%')) OR (#{searchText} REGEXP '^[a-zA-Z]+$' AND name like concat('%',#{searchText},'%'))) order by ${pageRequest.sort} ${pageRequest.desc} limit ${size} offset ${size*(pageRequest.page-1)} ) union ((select count(*) FROM employee WHERE attendance_manager = #{isManager} and (#{searchText} REGEXP '^[0-9]+$' AND employee_id like concat('%',#{searchText},'%') OR (#{searchText} REGEXP '^[a-zA-Z]+$' AND name like concat('%',#{searchText},'%')) ))) ")
